...
run() {
  ...
  local duration booked doubles failed
  duration=$(echo "$out" | grep "Duration:"      | awk '{print $2}')
  booked=$(  echo "$out" | grep "Booked:"        | awk '{print $2}')
  doubles=$( echo "$out" | grep "Double-booked:" | awk '{print $2}')
  failed=$(  echo "$out" | grep "Failed:"        | awk '{print $2}')
  failed="${failed:-0}"
  printf "│ %-26s │ %8s │ %10s │ %7s │ %6s │\n" \
    "$label" "$duration" "$booked" "$doubles" "$failed"
}
...
echo
echo "┌────────────────────────────┬──────────┬────────────┬─────────┬────────┐"
echo "│ Approach                   │ Duration │ Booked/120 │ Doubles │ Failed │"
echo "├────────────────────────────┼──────────┼────────────┼─────────┼────────┤"
